function [F] = layered(cortex, nbs, memberships, h, ind, N, contrast, cind, fPan, fDim, fInds, ROI, origin, strcont)
...
%         title(ax(2,1), "All Components", 'FontSize',10);
        ...
        legend(ax(1,1), s, scont, 'Location','southoutside', 'Color','w', 'FontSize',10);   %  'Orientation','horizontal',
        clear s lgnd s2
        
%         % Generate directed graphs for all components
%         ax(1,1) = subplot(fPan(1), fPan(2), fInds{1}); axis off		% graph format
%         g = cell(1, size(map,2));
%         G = plot([],[]);
%         for c2 = 1:size(map,2)
...
%                 g = digraph(map{c2}{f}, ROI{:,'Label'});
% 
%                 % Isolate nodes & links of interest
%                 [r,cl] = find(map{c2}{f});
%                 S = subgraph(g, ROI{:,'Label'}(union(r,cl)));
% 
%                 % Plot directed graphs per component
